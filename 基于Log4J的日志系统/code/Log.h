...
public:
    ...
    bool reopen();//打开日志文件
    string toyamlstring(){
        cout<<"loggerlevel:"<<LogLevel::ToString(level)<<endl;
        YAML::Node n;
        stringstream ss;
        n["type"]="FileAppender";
        n["file"]=file_name;
...
public:
    ...
    string GetFormate() { return formate->GetFormate(); }
    void SetLevel(LogLevel::Level level) { this->level = level; cout<<"level:"<<level<<endl;}
    ...
    string toyamlstring(){
        ...
        for(auto&o:appenders){
           n["appernders"].push_back(YAML::Load(o->toyamlstring()));
        }
        stringstream ss;
        ss<<n;
        return ss.str();
...
public:
    ...
    LoggerMes() {
        ...
        //root->AddAppender(LogAppender::ptr(new SmtpAppeder("smtp.qq.com","185334690@qq.com","ysidtuhpbiwxbhdg",mps)));
    ...
    string toyamlstring(){
        ...
        YAML::Node n;
        for(auto &it:mp){
            n.push_back(YAML::Load(it.second->toyamlstring()));
